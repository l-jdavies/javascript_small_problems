...
function longestSentence(text) {
  let splitSentences = text.split(/[!.?]/g);
  let wordsOnly = returnWordsOnly(splitSentences);
  let longSentence = findLongestSentence(wordsOnly);

  console.log(longSentence.join(' '));
  console.log(' ');
  console.log(`The longest sentence has ${longSentence.length} words.`);
}

function returnWordsOnly(array) {
  let wordPattern = /[^.!? ]/gi;
  return array.map(sentence => sentence.split(' ')).map(subArr => subArr.filter(word => word.match(wordPattern)));
}
